...
export default class History extends Component {
  ...
  render({}, { items }) {
    ...
    if (items.length === 0) child = <p>History is blank</p>
      ...
      child = sorted.filter((item) => item.scores.length === 2).map((item) =>
        <tr key={item.gameId}>
          <td class={style.cell}>{item.users[0]}</td>
          <td class={style.cell}>{item.users[1]}</td>
          <td class={style.cell}>{item.scores.find((x) => x.user === item.users[0]).count}</td>
          <td class={style.cell}>{item.scores.find((x) => x.user === item.users[1]).count}</td>
